...
DROP FUNCTION IF EXISTS insert_session_for_device(TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, INTEGER, INTEGER, INTEGER);
DROP FUNCTION IF EXISTS get_programs_for_device(TEXT);
DROP FUNCTION IF EXISTS get_sessions_for_device(TEXT);

-- Watch API Functions v2
-- Fixed: Don't check expires_at for authorized devices (once paired, stays paired)
...
BEGIN
    ...
    -- NOTE: expires_at only applies to pairing codes, not authorized devices
...
COMMENT ON FUNCTION get_programs_for_device IS 'Get programs for an authorized device (bypasses RLS) - v2: no expiration check';
COMMENT ON FUNCTION get_sessions_for_device IS 'Get sessions for an authorized device (bypasses RLS) - v2: no expiration check';
COMMENT ON FUNCTION insert_session_for_device IS 'Insert session for an authorized device (bypasses RLS) - v2: no expiration check';
